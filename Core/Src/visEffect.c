...
// RGB Framebuffers
uint8_t frameBuffer[3*100];
uint8_t frameBuffer2[3*100];
...
void visDots(uint8_t *frameBuffer, uint32_t frameBufferSize, uint32_t random, uint32_t fadeOutFactor)
{
	...
	for( i = 0; i < frameBufferSize / 3; i++)
	{
		...
		if(rand() % random == 0)
		{
			frameBuffer[i*3 + 0] = 50;
			frameBuffer[i*3 + 1] = 50;
			frameBuffer[i*3 + 2] = 50;
		}
...
//		visRainbow(frameBuffer, sizeof(frameBuffer), 40);
